		...
		if(!expr.joinflag)
			...
			case AND: {
				...
				if(lhs.size()!=0 && rhs.size()!=0){
					...
					case FLOAT:
					case STRING: 
						Iterator itlhs=lhs.iterator();
						Iterator itrhs=rhs.iterator();
						
//						while(itlhs.hasNext()){
//							Datum[]d=(Datum[]) itlhs.next();
//							System.out.println("here....");
//							while(itrhs.hasNext()){
//								Datum[] d1=(Datum[]) itrhs.next();
//								if(compare(d, d1)){
//									ret.add(d);
//									break;
//								}
...
//						}
					...
					if(rhs.size()==0){
						Iterator itlhs= lhs.iterator();
						while(itlhs.hasNext()){
							ret.add((Datum[]) itlhs.next());
						}
					}
					else if(lhs.size()==0){
						Iterator itrhs= rhs.iterator();
						while(itrhs.hasNext()){
							ret.add((Datum[]) itrhs.next());
						}
					}
				}
				break;
